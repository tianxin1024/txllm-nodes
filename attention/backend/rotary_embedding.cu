...
public:
    YarnImpl(const core::Context &ctx, model::ModelConfig cfg) :
        ...
        low = std::max(low1, 0.); // Clamp values just in case
        high = std::min(high1, dim_head - 1.);

        _mscale = yarn_get_mscale(scaling_factor) * attn_factor;
        if (cfg.model_type == "deepseek_v2") {
            _mscale = yarn_get_mscale(scaling_factor, mscale) / yarn_get_mscale(scaling_factor, mscale_all_dim) * attn_factor;
        }
