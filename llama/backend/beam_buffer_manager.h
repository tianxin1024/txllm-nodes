...
public:
    ...
    int get_hypo_pos(int hypo_id) const {
        return head_placement_[hypo_id];
    };
    void increase_buf_ref(int hypo_end_pos) {
        buf_local[hypo_end_pos].ref_count++;
    }
    void decrease_buf_ref(int hypo_end_pos) {
        buf_local[hypo_end_pos].ref_count--;
    }
    void increase_hypo_ref(int hypo_id) {
        increase_buf_ref(get_hypo_pos(hypo_id));
    }

    bool full() const {
        return unused_buffer_pos.empty();
    };
...
public:
    ...
    void get_hypothesis_tokens(int hypo_last_pos, std::vector<TokenT> *tokens, bool reverse = false) const {
        // 往回遍历该 hypothesis 所有的 token；不包含 input
        int pos = hypo_last_pos;
        while (pos > last_input_buf_pos) {
            int token_id = buf_local[pos].token;
        ...
        BM_ASSERT(pos >= 0, "position overflow!");
        if (!reverse) {
            // 转成正序
            std::reverse(tokens->begin(), tokens->end());
        }
